...
function FP_Cat_model_2(ESI; allowplots=false, allowsave=false, showph=false)
    ...
    if allowplots == true
        ...
        if showph == true
            ...
            plot_pH = scatter(y_train[:,2],z4,label="Training set", legend=:best, title = "ESI$(ESI_symbol) IEs from FP", markershape = :circle, marker_z = y_train[:,1] , xlabel = "Experimental log(IE)", ylabel = "Predicted log(IE)",color=:jet)
            scatter!(y_test[:,2],z5,label="Test set", marker_z = y_test[:,1] , markershape = :rect,color=:jet)
            plot!([minimum(vcat(y_train[:,2],y_test[:,2])),maximum(vcat(y_train[:,2],y_test[:,2]))],[minimum(vcat(y_train[:,2],y_test[:,2])),maximum(vcat(y_train[:,2],y_test[:,2]))], label="1:1 line",width=2)
            if allowsave == true
                savefig("C:\\Users\\alex_\\Documents\\GitHub\\IE_prediction\\Graphs\\Fingerprints\\CatBoost\\Cat_Regression_pHcolor_M2M4_$ESI_name.png")
            ...
            plot_pH_res = scatter(y_train[:,2],z6,label="Training set", legend=:best, title = "ESI$(ESI_symbol) Regression residuals",markershape=:circle, marker_z=y_train[:,1],color = :jet, xlabel = "Experimental log(IE)", ylabel = "Residual")
            scatter!(y_test[:,2],z7, markershape=:rect,marker_z=y_test[:,1], label="Test set",color=:jet)
            plot!([minimum(vcat(y_test[:,2],y_train[:,2])),maximum(vcat(y_test[:,2],y_train[:,2]))],[0,0],label="1:1 line",width=2) # 1:1 line
            plot!([minimum(vcat(y_test[:,2],y_train[:,2])),maximum(vcat(y_test[:,2],y_train[:,2]))],[3*std(vcat(z6,z7)),3*std(vcat(z6,z7))],label="+/- 3 std",linecolor ="grey",width=2) # +3 sigma
            plot!([minimum(vcat(y_test[:,2],y_train[:,2])),maximum(vcat(y_test[:,2],y_train[:,2]))],[-3*std(vcat(z6,z7)),-3*std(vcat(z6,z7))],label=false,linecolor ="grey",width=2) #-3 sigma
...
importance_pos, accuracy_tr_pos, accuracy_te_pos, y_hat_train_pos, y_hat_test_pos, res_train_pos, res_test_pos = FP_Cat_model_2(+1, allowplots=true, allowsave=false,showph=false);


## Australia data ## 
function FP_Cat_model_2_aus(ESI, X_aus; allowplots=false, allowsave=false, showph=false)
    if ESI == -1
        ESI_name = "neg"
        ESI_symbol = "-"
        n_trees = 1200
...
function pH_investigation_aus(ESI, aus_raw)
    aus_FP = Matrix(aus_raw[:,6:end])
    pH_targets = [2.7,2,4,6,7,8,10,12]      # The first value should be the most probable pH used in the measurements

    aus_IEs_all_pHs = zeros(size(aus_FP,1), length(pH_targets))
    for i = 1:length(pH_targets)
        aus_i = hcat(pH_targets[i] .* ones(size(aus_FP,1)), aus_FP)
        z1,z2,z3,z4,z5,z6,z7,aus_IEs_all_pHs[:,i] = FP_Cat_model_2_aus(ESI, aus_i)
        println(pH_targets[i])
    end

    # Extracted variables
    lowest_IEs = DataFrame(Low_IE_boundary = minimum(aus_IEs_all_pHs,dims=2)[:])
    highest_IEs = DataFrame(High_IE_boundary = maximum(aus_IEs_all_pHs,dims=2)[:])
    original_IEs = DataFrame(Predicted_IE = aus_IEs_all_pHs[:,1])
    pred_lo_hi = hcat(hcat(original_IEs,lowest_IEs),highest_IEs)
    final = hcat(aus_raw[:,1:5], pred_lo_hi)

    return final
end

aus_raw = CSV.read("C:\\Users\\alex_\\Documents\\GitHub\\IE_prediction\\Australia data\\aus_FP-12_fromsmiles.csv", DataFrame)
aus_pred_IEs = pH_investigation_aus(+1, aus_raw)
CSV.write("C:\\Users\\alex_\\Documents\\GitHub\\IE_prediction\\Australia data\\aus_predicted_IEs.csv", aus_pred_IEs)