...
function create_df_mode(data_mode::String; allowplots::Bool=false, allowsave::Bool=false)
    ...
    if allowplots == true
        ...
        plot!([0,0],[minimum(df_mode_residuals_CNL),maximum(df_mode_residuals_CNL)],label=false,width=2,dpi=300, c="green")
        plot!([minimum(df_mode_residuals_FP),maximum(df_mode_residuals_FP)],[0,0],label=false,width=2,dpi=300, c="green")
        display(p2)
        if allowsave == true
