...
function create_df_mode(data_mode::String; allowplots::Bool=false, allowsave::Bool=false)
    ...
    #Plots
    if allowplots == true
        p1 = scatter(df_mode[:,"IE_hat_FP"],df_mode[:,"IE_hat_CNL"],label=false, alpha=0.8, xlabel="Structure-based predicted logIE", ylabel="MS-based predicted logIE", legend=:best, c="magenta",dpi=300)
        plot!([minimum(df_mode[:,"IE_hat_FP"]),maximum(df_mode[:,"IE_hat_FP"])],[minimum(df_mode[:,"IE_hat_FP"]),maximum(df_mode[:,"IE_hat_FP"])],label="1:1 line",width=2,dpi=300, c="green")
        display(p1)
        if allowsave == true
            savefig("C:\\Users\\alex_\\Documents\\GitHub\\IE_prediction-project\\Unified\\Graphs\\CNL vs FP\\FPpred_v_CNLpred$(data_mode).png")
        end
        p2 = scatter(df_mode_residuals_FP, df_mode_residuals_CNL,label=false, alpha=0.8, xlabel="Structure-based predicted logIE residuals", ylabel="MS-based predicted logIE residuals", legend=:best, c="magenta",dpi=300)
        plot!([minimum(df_mode_residuals_FP),maximum(df_mode_residuals_FP)],[minimum(df_mode_residuals_FP),maximum(df_mode_residuals_FP)],label="1:1 line",width=2,dpi=300, c="green")
        display(p2)
        if allowsave == true
