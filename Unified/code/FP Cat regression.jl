...
opt = CSV.read("C:\\Users\\alex_\\Documents\\GitHub\\IE_prediction-project\\Unified\\data\\FP_optimization.csv", DataFrame)
sort(opt[:,:accuracy_test])

best_grades = groupby(opt, :FP_type) |> x -> combine(x, :accuracy_test => maximum => :Highest_TestSet_R2)
