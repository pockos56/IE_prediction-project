...
function Stratified_CNL_Classification_wFiltering_wConsensus_TestOnlyFiltered(ESI; consensus_threshold, allowplots::Bool=false, allowsave::Bool=false, showph::Bool=false, split_size::Float64=0.2, min_CNLs::Int=4, random_seed::Int=3, removeminusones::Bool=true, consensus_algorithm="replacement")
    function split_classes(ESI, classes; random_state::Int=random_seed, split_size::Float64=0.2)
        ...
        elseif ESI == 1
            ...
            threshold_up = 3
            threshold_down = 1   
            reg = RandomForestClassifier(n_estimators= 600, min_samples_leaf = 8, oob_score=true, n_jobs=-1, random_state=1)
        else error("Set ESI to -1 or +1 for ESI- and ESI+ accordingly")
        end
        ...
        if allowplots == true
            ...
            plot!([minimum(vcat(y_train,y_test)),threshold_up],[maximum(vcat(y_train,y_test)),threshold_up],label="High threshold",width=2)
            plot!([minimum(vcat(y_train,y_test)),threshold_down],[maximum(vcat(y_train,y_test)),threshold_down],label="Low threshold",width=2)
            ...
            if allowsave == true
                savefig("C:\\Users\\alex_\\Documents\\GitHub\\IE_prediction\\Graphs\\CNL\\CNL_Cat_Classification_Consensus_$ESI_name.png")
            end

            p4 = scatter(y_train,z6,label="Training set", legend=:best, title = "ESI$(ESI_symbol) Classification residuals", color = :magenta, xlabel = "Experimental log(IE)", ylabel = "Residual")
            ...
            if allowsave == true
                savefig("C:\\Users\\alex_\\Documents\\GitHub\\IE_prediction\\Graphs\\CNL\\CNL_Cat_Residuals_Classification_$ESI_name.png")
            end
            display(p123)
            display(p456)
            if showph == true           
                ...
                annotate!(maximum(vcat(y_train,y_test)),0.3+minimum(vcat(y_train,y_test)),latexstring("Test: R^2=$(round(z3, digits=3))"),:right)  
                    if allowsave == true
                    savefig("C:\\Users\\alex_\\Documents\\GitHub\\IE_prediction\\Graphs\\CNL\\CNL_Cat_Classification_pHcolor_Consensus_$ESI_name.png")
                end

                plot_pH_res = scatter(y_train,z6,label="Training set", legend=:best, title = "ESI$(ESI_symbol) Classification residuals",markershape=:circle, marker_z=X_train[:,1],color = :jet, xlabel = "Experimental log(IE)", ylabel = "Residual")
                ...
                if allowsave == true
                    savefig("C:\\Users\\alex_\\Documents\\GitHub\\IE_prediction\\Graphs\\CNL\\CNL_Cat_Residuals_pHcolor_Classification_$ESI_name.png")
                end
                display(plot_pH)
                display(plot_pH_res)
            end
...
important_variables_pos,accuracy_train_pos,accuracy_test_pos,y_hat_train_pos,y_hat_test_pos = Stratified_CNL_Classification_wFiltering_wConsensus_TestOnlyFiltered(+1, consensus_threshold=0.1)

countmap(y_hat_test_pos)
countmap(y_hat_train_pos)
