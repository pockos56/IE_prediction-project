...
smiles_ = unique(convert.(String, Internal[:,:SMILES]))[1:2000]

fp_vec1 = from_smiles_batch(smiles_, 25, fingerprints=true, descriptors=false)
CSV.write("C:\\Users\\alex_\\Documents\\GitHub\\IE_prediction-project\\FP recalculation\\FP_APC1.csv", fp_vec1)

fp_vec2 = from_smiles_batch(smiles_, 25, fingerprints=true, descriptors=false)
CSV.write("C:\\Users\\alex_\\Documents\\GitHub\\IE_prediction-project\\FP recalculation\\FP_APC2.csv", fp_vec2)

fp_vec3 = from_smiles_batch(smiles_, 25, fingerprints=true, descriptors=false)
...
all_unstable_colnames = unique(vcat(vcat(vcat(names(fp1[:,falses_cols]),names(fp_apc1[:,falses_cols_12])),names(fp_apc2[:,falses_cols_13])),names(fp_apc3[:,falses_cols_23])))
...
all_unstable_comps = unique(vcat(vcat(falses_comps,falses_comps_12),vcat(falses_comps_13,falses_comps_23)))
